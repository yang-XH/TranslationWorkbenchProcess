    ...
    for final_file in YS_final_files:
        if os.path.exists(os.path.join(YS_final_path, final_file)):
            for file in listdir_nohidden(os.path.join(YS_final_path, final_file)):
                if not file.endswith(".xls") or file in YS_file_ignore:
                    # xlsx/xls与csv文件内容相同，只需遍历其中一个，且还有zip文件等
                    # YS_file_ignore中的内容不需遍历到词典中
                    continue
...
if __name__ == '__main__':
    ...
    YS_file_ignore = excel_config['YS_file_ignore']
    ...
    file_to_field_dict = get_YS_final_dict(YS_final_path, YS_final_files, YS_file_ignore, YS_dict_txt_path, field_app_to_be_confirmed_txt_path, index0, index1)
